...
if (isset($_REQUEST['payment_type'])) { $payment_type = $_REQUEST['payment_type']; }				// тип оплаты	
	...
	$db->query('UPDATE history SET payment = "'.$sum.'" WHERE id = "'.$number_request.'"'); 
			...
			for ($i = 0; $i <= count($counter_request)-2; $i++)
				{	
					...
					foreach ($read_request as $request_field)
						{
							$payment = $request_field['FULL_RATE'] - $request_field['BALANCE'];
							$full_rate = $request_field['FULL_RATE'];
						}
						
					$db->query('UPDATE hotel_accounts SET BALANCE = "'.$full_rate.'" WHERE SITENUMBER = "'.$counter_request[$i].'"');   // перезаполняем баланс на полную сумму
