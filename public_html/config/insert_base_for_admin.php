...
$passport_series = $_POST['passport_series'];						// считываем серию паспорта
$passport_number = $_POST['passport_number'];						// считываем номер паспорта
$passport_who_issue = $_POST['passport_who_issue'];					// кем выдан
$date_issue = $_POST['date_issue'];									// дата выдачи

if (isset($_POST['sum_confirm']))
	{
...
if (isset($_POST['payment_manual']))
	{
		$payment_manual = $_POST['manual_input'];
		if ($payment_manual == '0')
			{
				$sum = 0;										// предохраняемся от случайного реагирования submit
			}
		if ($sum != 0)
			{
				$prepayment = $payment_manual/$sum;
			}
		else
			{
				$prepayment = 1;
...
if ($switch == 1)											// ... если есть обновляем данные - добавляем
	{
		...
		$db->query('UPDATE user SET passport_series = "'.$passport_series.'" WHERE id = "'.$user_number.'"'); 
		$db->query('UPDATE user SET passport_number = "'.$passport_number.'" WHERE id = "'.$user_number.'"');
		$db->query('UPDATE user SET passport_who_issue = "'.$passport_who_issue.'" WHERE id = "'.$user_number.'"');
		$db->query('UPDATE user SET date_issue = "'.$date_issue.'" WHERE id = "'.$user_number.'"');
	}
	


if ($sum != '0')                                                   // если сумма не равна нулю - оформляем заявку
	{
		...
		echo '<center><a style = "color: #fff; font-size: 18px; font-family: pragmatica;" >'.$number_house.'</a></center>';
	}
?>