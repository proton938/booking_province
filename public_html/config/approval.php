...
if (isset($_REQUEST['payment_type'])) { $payment_type = $_REQUEST['payment_type']; }				// тип оплаты
	...
	foreach ($read_number as $number_field)
		{
			for ($i = 1; $i <= count($booking)-1; $i++)				  // выводим брони всех номеров в доме
				{
							...
							if ($number_field['year']==$year && $number_field['month']==$month && $number_field['date']==$date)
								{
									...
									$db->query('UPDATE numbers SET number_'.$hotel_number.' = "1" WHERE id = "'.$number_field['id'].'"');  	
								}
						}
				}
		}
	...
	$history_bufer =  $db->query('SELECT * FROM history WHERE id = "'.$number_request.'"');			// обращаемся к таблице истории пользователей по id 
	$read_history = $history_bufer->fetchAll();

	$db->query('UPDATE history SET approval = "on" WHERE id = "'.$number_request.'"'); 
	...
	foreach($read_history as $history_field)													// выводим номера заявок
		{
			...
			for ($i = 0; $i <= count($counter_request)-2; $i++)
				{	
					...
					$db->query('INSERT INTO hotel_accountpayments (	SUCCESS,
																	...
																	VALUES
																		  (	"1",
																			...
																			"'.$date_today.'",
																			"'.$payment_type.'",
																			"Оплата через панель управления",
																			"'.$payment.'",
																			"11"
																				)
																				');
					...
					$COMMENT->appendChild($dom->createTextNode('Оплата через панель управления'));
