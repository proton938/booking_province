...
<tr style = "vertical-align: top;">
	...
	<form method = "POST" enctype = "multipart/form-data" action = "../config/insert_base_for_admin.php" target = "status">
		...
		<strong>
			...
			<table style = "float: left;" >
				...
				<tr>
					<td>
						<input type = "text" id = "surname" name = "surname" style = "border: solid 1px #CAA79C; border-style: inset; float: left; height: 25px; width: 100px; margin-top: 5px;" required> 
					</td>
					<td style = "padding-left: 20px;" >
						<input type = "text" id = "name" name = "name" style = "border: solid 1px #CAA79C; border-style: inset; float: left; height: 25px; width: 100px; margin-top: 5px;" required> 
					</td>
					<td style = "padding-left: 20px;" >
						<input type = "text" id = "patronymic" name = "patronymic" style = "border: solid 1px #CAA79C; border-style: inset; float: left; height: 25px; width: 100px; margin-top: 5px;" required> 
					</td>
				</tr>
			</table>
			<br><br><br><br>
			...
			<table>
				...
				<tr>
					<td>			
						<input type = "text" id = "telephone" name = "telephone" style = "border: solid 1px #CAA79C; border-style: inset; float: left; height: 25px; margin-top: 5px;" required><a style = "opacity: 0;"></a> 
					</td>
					<td style = "padding-left: 20px;" >
						<input type = "text" id = "email" name = "email" style = "border: solid 1px #CAA79C; border-style: inset; float: left; height: 25px; margin-top: 5px;" >
					</td>
				</tr>
			</table>
			...
			<table>
				...
				<tr style = "vertical-align: top;">
					<td>			
						<textarea rows="5" id = "passport_who_issue" name = "passport_who_issue" style = "border: solid 1px #fff; border-style: inset; float: left; margin-top: 5px; resize: none; " ></textarea><a style = "opacity: 0;"></a> 
					</td>
					<td style = "padding-left: 20px;" >
			...
			<input id = "sum_confirm" name = "sum_confirm" type = "hidden" value = "0" class = "noredakt_text" style = "background: rgba(113, 76, 70, 1); padding-top: 5px; padding-bottom: 5px; text-align: right; float: right; border: solid 1px #CAA79C; border-style: inset;" readonly=true>
			
			<input style = "float: right;" type = "submit" class = "number_select" value = "Утвердить" >
	...
	<div style = "width: 500px; height: 780px; overflow: auto;">
			...
			if ($history_field['user_id'] == $user_field['id'])
				{	
							...
							foreach ($read_request as $request_field)
								{
									...
									echo '	
										<div style = "background: #fff; border: dotted 1px #bbb; whidth: 600px;">
										<br>
									...
									if ($request_field['FROM_THE_SITE'] == 1 && $request_field['APPROVAL'] == '')									// если блок заявок был отправлен с сайта
										{
											echo '
												<input 	type = "button" 
														style = " 
																...
																border: none; padding: 3px 10px 3px 10px; 
																border-radius: 5px;
																cursor: pointer;" value = "Утвердить вручную"
																onclick = "load_approval_once_'.$user_number.'_'.$history_field['id'].'_'.$request_field['id'].'()"
													>
												<script>
													function load_approval_once_'.$user_number.'_'.$history_field['id'].'_'.$request_field['id'].'()
														{
															$("#load_approval").load("../config/approval_once.php", "user_number='.$user_number.'&id_block_request='.$history_field['id'].'&number_request='.$counter_request[$i].'");
														}
												</script><br><br>';
										}
					...
					echo	'
							...
							<br><br>
							<a style = "color: red; ">
								Заявлено на предоплату: <strong>'.$history_field['payment'].'</strong>
							</a>
							<br><br>
							<a  style = "color: red; ">
								Остается: <strong>'.$full_residue.'</strong>
							</a>';
...
<div style = "position: relative;" id = "load_approval">
</div>
