...
if (isset($_REQUEST['booking'])) { $booking = $_REQUEST['booking']; }								// содержимое заявки
							...
							if ($number_field['year']==$year && $number_field['month']==$month && $number_field['date']==$date)
								{
									...
									if ($number_field['number_'.$hotel_number] != '' && $number_field['user_'.$hotel_number] != $user_number)
										{
											$switch = 1;
										}
									if ($switch == 0)
										{
											// отмена бронирования - вводим "" в соответствующее поле
											$db->query('UPDATE numbers SET number_'.$hotel_number.' = "" WHERE id = "'.$number_field['id'].'"');  
											$db->query('UPDATE numbers SET user_'.$hotel_number.' = "" WHERE id = "'.$number_field['id'].'"');
										}
								}
						}
				}
	...
	$db->query('UPDATE history SET approval = "no" WHERE id = "'.$number_request.'"'); 
